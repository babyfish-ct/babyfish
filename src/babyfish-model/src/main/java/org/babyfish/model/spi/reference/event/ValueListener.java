...
public interface ValueListener<T> extends EventListener {
    ...
    default void modifying(ValueEvent<T> e) throws Throwable {}
    
    default void modified(ValueEvent<T> e) throws Throwable {}
    
    static <T> ValueListener<T> combine(ValueListener<T> a, ValueListener<T> b) {
        throw new UnsupportedOperationException("Instrument required");
    }
    
    static <T> ValueListener<T> remove(ValueListener<T> a, ValueListener<T> b) {
        throw new UnsupportedOperationException("Instrument required");
    }
}
